...
const OnboardingPage: React.FC = () => {
  ...
  // Helper: Save partial onboarding data (does not mark as complete)
  const savePartialOnboarding = (data: Partial<UserSkillData>) => {
    const prev = DataManager.getUserSkillData() || {};
    localStorage.setItem('userSkillData', JSON.stringify({ ...prev, ...data }));
  };

  const handleNext = () => {
    if (currentStep < ONBOARDING_STEPS.length - 1) {
      setCurrentStep(currentStep + 1);
      // Save partial data on every next
      savePartialOnboarding(formData);
    }
  };
  ...
  const handleSubmit = async () => {
    ...
    DataManager.completeOnboarding(formData as UserSkillData);
    // Immediately go to roadmap generation after setup
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-blue-50 to-purple-50 flex items-center justify-center px-2 md:px-8 lg:px-16">
